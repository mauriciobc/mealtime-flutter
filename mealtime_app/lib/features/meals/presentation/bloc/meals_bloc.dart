...
class MealsBloc extends Bloc<MealsEvent, MealsState> {
  ...
  Future<void> _onCreateMeal(CreateMeal event, Emitter<MealsState> emit) async {
    ...
    result.fold((failure) => emit(MealsError(failure)), (newMeal) {
      if (currentState is MealsLoaded) {
        final updatedMeals = [...currentState.meals, _toViewModel(newMeal)];
        emit(
          MealOperationSuccess(
            message: 'Refeição criada com sucesso!',
            meals: updatedMeals,
            updatedMeal: _toViewModel(newMeal),
          ),
        );
      } else {
        emit(
    ...
    result.fold((failure) => emit(MealsError(failure)), (updatedMeal) {
      if (currentState is MealsLoaded) {
        final updatedMeals = currentState.meals.map<MealViewModel>((vm) {
          return vm.meal.id == updatedMeal.id ? _toViewModel(updatedMeal) : vm;
        }).toList();
        emit(
          MealOperationSuccess(
            message: 'Refeição atualizada com sucesso!',
            meals: updatedMeals,
            updatedMeal: _toViewModel(updatedMeal),
          ),
        );
      } else {
        emit(
          MealOperationSuccess(
            message: 'Refeição atualizada com sucesso!',
            meals: [_toViewModel(updatedMeal)],
            updatedMeal: _toViewModel(updatedMeal),
          ),
        );
      }
  ...
  Future<void> _onCompleteMeal(
    ...
    result.fold((failure) => emit(MealsError(failure)), (completedMeal) {
      if (currentState is MealsLoaded) {
        final updatedMeals = currentState.meals.map<MealViewModel>((vm) {
          return vm.meal.id == completedMeal.id ? _toViewModel(completedMeal) : vm;
        }).toList();
        emit(
          MealOperationSuccess(
            message: 'Refeição concluída com sucesso!',
            meals: updatedMeals,
            updatedMeal: _toViewModel(completedMeal),
          ),
        );
      } else {
        emit(
          MealOperationSuccess(
            message: 'Refeição concluída com sucesso!',
            meals: [_toViewModel(completedMeal)],
            updatedMeal: _toViewModel(completedMeal),
          ),
        );
      }
  ...
  Future<void> _onSkipMeal(SkipMeal event, Emitter<MealsState> emit) async {
    ...
    result.fold((failure) => emit(MealsError(failure)), (skippedMeal) {
      if (currentState is MealsLoaded) {
        final updatedMeals = currentState.meals.map<MealViewModel>((vm) {
          return vm.meal.id == skippedMeal.id ? _toViewModel(skippedMeal) : vm;
        }).toList();
        emit(
          MealOperationSuccess(
            message: 'Refeição pulada com sucesso!',
            meals: updatedMeals,
            updatedMeal: _toViewModel(skippedMeal),
          ),
        );
      } else {
        emit(
          MealOperationSuccess(
            message: 'Refeição pulada com sucesso!',
            meals: [_toViewModel(skippedMeal)],
            updatedMeal: _toViewModel(skippedMeal),
          ),
        );
      }
  ...
  // Performance: Convert to ViewModel to pre-compute formatted strings,
  // avoiding repeated calculations in widgets.
